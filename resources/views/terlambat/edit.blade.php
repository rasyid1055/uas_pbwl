    ...
    <div class="row justify-content-center">
        <div class="col-md-5">
            <div class="card bg-dark">
                <div class="card-header text-light">{{ __('Edit Data Keterlambatan') }}</div>
                <div class="card-body">
                <form action="/terlambat/{{$terlambat->id_terlambat}}" method="POST">
                @method('put')
                @csrf
                <br>
                <div class="row">
                    ...
                    <select class="form-select" aria-label="Default select example" name="id_piket" id="id_piket">
                      @foreach($guru as $g)
                      <option value="{{ $g->id_piket }}" selected>{{ $g->nama_guru }}</option>
                      @endforeach
                    </select>
                    </div>
                    </div> <br>
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="tgl_terlambat" name="tgl_terlambat" value="{{$terlambat->tgl_terlambat}}">
                    </div>
                    <div class="mb-3">
                        <label for="tgl_masuk" class="form-label">Tanggal Masuk</label><span style="color: red;"> *</span>
                        <input type="date" class="form-control" id="tgl_masuk" name="tgl_masuk" value="{{$terlambat->tgl_masuk}}">
                    </div>
                    <div class="mb-3">
                        <label for="jam_terlambat" class="form-label">Jam Terlambat</label><span style="color: red;"> *</span>
                        <input type="time" class="form-control" id="jam_terlambat" name="jam_terlambat" value="{{$terlambat->jam_terlambat}}">
                    </div>
                    <div class="mb-3">
                        <label for="jam_masuk" class="form-label">Jam Masuk</label><span style="color: red;"> *</span>
                        <input type="time" class="form-control" id="jam_masuk" name="jam_masuk" value="{{$terlambat->jam_masuk}}">
                    </div>
                    <div class="mb-3">
                        <label for="hukuman" class="form-label">Hukuman</label><span style="color: red;"> *</span>
                        <input type="text" class="form-control" id="hukuman" name="hukuman" value="{{$terlambat->hukuman}}">
                    </div>
                    <div class="mb-3">
                        <label for="alamat" class="form-label">Alasan</label> <span style="color: red;"> *</span></label>
                        <textarea class="form-control" id="alasan" name="alasan" rows="3" >{{$terlambat->alasan}}</textarea>
                </div>
